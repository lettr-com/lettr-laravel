...
describe('generated code style', function () {
    beforeEach(function () {
        serveTemplates($this, [
            'welcome-email' => ['name' => "Don't miss out", 'tags' => [new MergeTag(key: 'first_name', required: true), new MergeTag(key: 'company', required: false)]],
            'order' => ['tags' => [
                new MergeTag(key: 'items', required: true, children: [new MergeTagChild(key: 'name')]),
                new MergeTag(key: 'notes', required: false, children: [new MergeTagChild(key: 'text', type: 'integer')]),
            ]],
            'plain' => [],
        ]);
    });

    it('passes Pint with the Laravel preset for every kind of generated file', function (array $pull) {
        config()->set('lettr.templates.blade_path', $this->root.'/views');
        config()->set('lettr.templates.html_path', $this->root.'/html');

        $this->artisan('lettr:generate-enum')->assertSuccessful();
        $this->artisan('lettr:pull', ['--with-mailables' => true, ...$pull])->assertSuccessful();

        // Pint reads the PHP files only; the Blade views and HTML aren't PHP classes
        exec('rm -rf '.escapeshellarg($this->root.'/views').' '.escapeshellarg($this->root.'/html'));
        exec(escapeshellarg(__DIR__.'/../../vendor/bin/pint').' --test --preset laravel '.escapeshellarg($this->root).' 2>&1', $output, $code);

        expect(generatedFiles($this->root))->toHaveCount(8)
            ->and($code)->toBe(0, implode("\n", $output))
            ->and(generatedLoadError($this->root))->toBeNull();
    })->with([
        'Blade Mailables' => [[]],
        'Lettr template Mailables' => [['--as-html' => true]],
    ]);

    it('marks every generated class as generated and overwritten on the next run', function () {
        $this->artisan('lettr:generate-enum')->assertSuccessful();
        $this->artisan('lettr:pull', ['--with-mailables' => true, '--as-html' => true])->assertSuccessful();

        foreach (generatedFiles($this->root) as $file) {
            if (str_ends_with($file, '.php')) {
                expect(file_get_contents($this->root.'/'.$file))->toMatch('/\/\*\*(\n \* [^\n]+)+\n \*\n \* Generated by `php artisan lettr:[^`]+`[^\n]*\n( \* [^\n]*\n)* \*\/\n(final readonly class|class|enum) /');
            }
        }

        expect(file_get_contents($this->root.'/Dto/OrderDataNoteData.php'))->toContain('One item of the `notes` loop in the Lettr template `order`.')
            ->and(file_get_contents($this->root.'/Mail/Plain.php'))->not->toContain('__construct')->not->toContain('withMergeTags');
    });
});
